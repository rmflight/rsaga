...
initPop[2] <- runif(1, min=0, max=200)
neighborFunction <- function(currentPopulation, currentTemperature, alpha){
  ...
  newPopulation <- rnorm(length(currentPopulation), mean=currentPopulation, sd=scaleVariance)
...
sa <- function(initialSolution, evalFunction, neighborFunction, initialTemperature=300, deltaTemperature=1, temperatureStep=NA, alpha=1, sameTolerance=0.00001, nTry=10000){
  ...
  while ((currTemp > 0) && (currTol > sameTolerance) && (iTry < nTry)){
    newSolution <- neighborFunction(currSolution, currTemp, alpha)
...
```{r tryitOut}
initPop <- runif(8, min=0, max=200)
initPop[2] <- runif(1, min=0, max=1)
outSA <- sa(initPop, energyFunc, neighborFunction, 300, 0.01, alpha=0.01, nTry=Inf)
