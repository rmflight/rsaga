...
Our evaluation criteria is **how** different is a new set of `y` based on our candidate `beta` values.

```{r energyFunction}
# assumes that org and new are vectors of values
energyFunc <- function(org, new){
  nObs <- length(org)
...
And our values for each value of beta will be drawn uniformly from a distribution with a range of 0 to 200.

```{r initPop}
initPop <- runif(8, min=0, max=200)
neighborFunction <- function(population, )
```
